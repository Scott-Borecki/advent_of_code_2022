...
class PyroclasticFlow
  ...
  LAST_ROCK = 1000000000000.freeze
  ...
  def print_rock_tower(shape_coords = [])
    ...
    occupied_spaces.keys.each do |x, y|
      graph[y][x] = '#'
    end
  ...
  def spaces_occupied?(shape_coords)
    shape_coords.any? { |coord| occupied_spaces[coord] }
  end
  ...
  def place_shape(shape_coords)
    shape_coords.each { |coord| @occupied_spaces[coord] = true }
  end
